...
void do_bgfg(char **argv)
{
    ...
    int jid; /* holds return value and jid of job */
    pid_t pid;
    ...
    if (strcmp(argv[0], "bg") != 0 && !(job->state == ST || job->state == BG)) {
    ...
    // if (strcmp(argv[0], "bg") == 0) {
    //     /* BG: if its state is stoppped, continue in the background */
    //     if (job->state == ST) {
    ...
    //         job->state = BG;
    //     } else {
    //         printf("this job is already run in the background\n");
    //     }
    // }
    // else {
    //     if (kill(-pid, SIGCONT) < 0) {
...
void sigchld_handler(int sig)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        /* if exit normally, we can safely delete the job */
        if (WIFEXITED(status)) {
        ...
        if (WIFSTOPPED(status)) {
            printf("JOB [%d] (%d) stoppped by signal %d\n", pid2jid(pid), pid, WSTOPSIG(status));
        }
 ...
 *    to the foreground job.
 输入 ctrl-c会给当前的前台进程（包括其子进程）发送 SIGINT 信号
 */
void sigint_handler(int sig)
{
    int olderrno = errno;
